"""OpenAIOcrService 真实 API 调用集成测试。

需要设置环境变量 OPENAI_OCR_API_KEY 才能运行。
可选设置 OPENAI_OCR_BASE_URL 和 OPENAI_OCR_MODEL。

运行方式：
    uv run pytest tests/integration/test_openai_ocr_api.py -v -s
"""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="OPENAI_OCR_API_KEY 未设置，跳过集成测试",
)
...
@pytest.mark.asyncio
async def test_ocr_jing_rao(ocr_service: OpenAIOcrService) -> None:
...
@pytest.mark.asyncio
async def test_ocr_qi_zhi(ocr_service: OpenAIOcrService) -> None:
...
@pytest.mark.asyncio
async def test_ocr_jing_xiang(ocr_service: OpenAIOcrService) -> None:
...
@pytest.mark.asyncio
async def test_ocr_tian_ming(ocr_service: OpenAIOcrService) -> None:
    """测试：听天由命吧.png → 听天由命吧"""
