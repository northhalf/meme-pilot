...
with patch("nonebot.on_command", _mock_on_command):
    from bot.plugins import sim
    from bot.plugins.sim import handle_sim


class TestSimCommandRegistration:
    """测试 /sim 命令注册边界。"""
    ...
    def test_requires_whitespace_boundary(self) -> None:
        ...
        assert registration.args[0] == "sim"
...
class TestHandleSimAuth:
    ...
    @patch.object(sim, "dispatch_search_results", new_callable=AsyncMock)
