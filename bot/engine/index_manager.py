...
from typing import TYPE_CHECKING, AsyncIterator, Protocol, TypeVar, cast, overload
...
from .vector_store import VectorHit, VectorRecord
...
class _CollectionSyncStoreProtocol(Protocol):
    """刷新合集生命周期所需的元数据存储接口。"""
...
class _MetadataRestoreProtocol(Protocol):
    """刷新删除失败后恢复 SQLite 条目所需接口。"""
...
class _VectorCollectionSyncProtocol(Protocol):
    """刷新合集元数据一致性所需的向量存储接口。"""
...
class _VectorSnapshotProtocol(Protocol):
    """去重替换和移动补偿所需的向量快照接口。"""
...
class _MoveMetadataProtocol(Protocol):
    """移动编号分配所需的元数据接口。"""

    def find_next_local_id(self, collection_id: int) -> int: ...


class _MoveVectorProtocol(_VectorSnapshotProtocol, Protocol):
    """移动 Chroma 更新与完整快照补偿接口。"""
...
class MetadataStoreProtocol(Protocol):
    ...
    def set_selected_collection(
        self, scope: "ScopeLike", collection_id: int
    ) -> None: ...
    def close(self) -> None: ...
...
class IndexManager:
    ...
    def _preview_move_locked(
        ...
        *,
        ...
        local_id = cast(_MoveMetadataProtocol, self._metadata_store).find_next_local_id(
    ...
    async def _execute_move(self, req: _WriteRequest) -> MoveResult:
        ...
        local_id = await asyncio.to_thread(
            cast(_MoveMetadataProtocol, self._metadata_store).find_next_local_id,
            req.target_collection_id,
        )
        ...
        vector_store = cast(_MoveVectorProtocol, self._vector_store)
    ...
    async def _sync_phase0_consistency(self, failed: list[str]) -> None:
        ...
        vector_store = cast(_VectorCollectionSyncProtocol, self._vector_store)
    ...
    async def _sync_phase1_delete(self, existing: set[str], failed: list[str]) -> int:
        ...
        """
        entries = await asyncio.to_thread(self._metadata_store.get_all_entries)
        metadata_store = cast(_MetadataRestoreProtocol, self._metadata_store)
        deleted = 0
    ...
    async def _sync_collections_delete(
        ...
        """
        deleted = reset = 0
        metadata_store = cast(_CollectionSyncStoreProtocol, self._metadata_store)
    ...
    async def _write_entry(
        ...
        *,
        ...
        if old_id is not None:
            ...
            vector_store = cast(_VectorSnapshotProtocol, self._vector_store)
