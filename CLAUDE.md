...
按所选 provider 必填：
- `OPENAI_EMBEDDING_API_KEY`：`EMBEDDING_PROVIDER=openai`（默认）时必填，任意 OpenAI 兼容 Embedding 服务 API Key
