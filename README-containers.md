...
| `/cancel` | `/c` | 取消当前操作 | ✅ |
| `/collection create <名称>` | - | 创建表情包合集 | 私聊 |
...
cd meme-pilot
cp .env.example .env
# 编辑 .env 填入 QQ_ACCOUNT、AUTHORIZED_USER_IDS，
# 并按所选 Embedding/OCR provider 配置对应凭证
# Compose 仍要求 OPENAI_EMBEDDING_API_KEY 非空；google 模式可填 unused-for-google
# 把表情包图片（.jpg/.png/.gif/.webp/.bmp）放入 memes/ 目录（不存在时启动会自动创建）
docker compose up -d
docker compose logs -f bot
```

默认 Compose 使用 `northhalf/meme-pilot:latest`，每次启动都会检查并拉取最新发布镜像。Bot 运行时只使用 `EMBEDDING_PROVIDER` 选中的凭证；使用 Google 时，Compose 所需的 `OPENAI_EMBEDDING_API_KEY` 可填非空占位值，Bot 不会调用它。
...
- Embedding 由 `EMBEDDING_PROVIDER` 指定服务（默认 `openai`，OpenAI 兼容 API）。

## 镜像标签

- `northhalf/meme-pilot:latest`：最新发布版本对应的 tag 构建
