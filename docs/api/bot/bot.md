...
2. 通过 `provider_factory.create_ocr_provider(read_ocr_provider())` 与 `provider_factory.create_embedding_provider(read_embedding_provider())` 创建 OCR/Embedding 服务，以及 `RerankService`、`ImageOptimizer`；支持的 OCR 引擎：`paddle`（PaddleOCR 云 API）、`deepseek`（OpenAI 兼容 OCR，示例默认硅基流动 DeepSeek-OCR）、`rapidocr`（RapidOCR 本地 OCR）；支持的 Embedding 引擎：`openai`（OpenAI 兼容 Embedding，示例默认硅基流动 BAAI/bge-m3）、`google`（Google Embedding API）
...
| `EMBEDDING_API_KEY` | openai 时必填 | — | OpenAI 兼容 Embedding API Key（`EMBEDDING_PROVIDER=openai` 时必填） |
