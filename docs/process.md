...
- [x] `bot/engine/image_optimizer.py` — 图片无损压缩模块（Pillow 无损压缩，JPEG 去元数据+高质量重编码，PNG optimize=True，WebP lossless 模式，GIF 保留动画去冗余元数据；BMP 跳过返回 skipped；原子写入，压缩后变大时保留原文件）
