...
- 可选环境变量：`BOT_HOST`、`BOT_PORT`、`DEEPSEEK_BASE_URL`、`DEEPSEEK_MODEL`、`SILICONFLOW_BASE_URL`、`SILICONFLOW_OCR_MODEL`、`EMBEDDING_BASE_URL`、`EMBEDDING_MODEL`、`EMBEDDING_CONCURRENCY`（Embedding 并发上限，默认 5）、`OCR_CONCURRENCY`（OCR 并发上限，默认 5）、`RERANK_CONCURRENCY`（LLM 精排并发上限，默认 5）、`COMPRESS_CONCURRENCY`（图片压缩并发上限，默认 5）、`SESSION_EXPIRE_TIMEOUT`（会话超时，默认 60 秒）
- .env 不纳入版本控制
