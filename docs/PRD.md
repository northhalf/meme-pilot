...
Bot 接收 → 调用 KeywordSearcher.search("加班")
        │
        ├── rapidfuzz 只对 index.json 中的 OCR 文本计算相似度，不匹配文件名
        │    └── 过滤保留 similarity >= 60 的结果
        │
        ▼
        ├── 无结果 → "没有匹配到任何表情包 🙁"
        │
...
Bot 接收 → 调用 AIMatcher.match("加班心累")
        │
        ├── 阶段一：Embedding 语义搜索
        │   ├ 将用户描述向量化（SiliconFlow Embedding API，默认模型 `Qwen/Qwen3-Embedding-8B`）
...
6. 删除记录后保持其他已有 id 稳定，不重新编号，允许 `001`、`003` 这种临时编号空洞
...
12. `/refresh` 完成后回复摘要：新增数量、删除数量、失败数量；如有失败，最多列出前 10 个失败文件名
...
| 指标 | 要求 |
|------|------|
| OCR 首次建索引 | 100 张图 < 10 分钟（PaddleOCR 约 5s/张） |
...
| 图片 OCR 成功但识别不到文字 | `index.json` 中该条目的 `text` 写 "未识别到文字" |
| 单张新增图片 OCR 调用异常 | 跳过该图片，不写入索引；刷新继续处理其他图片，最终回复汇总失败文件列表 |
| PaddleOCR 初始化失败 | Bot 打印错误日志，回复"OCR 初始化失败"；本次刷新不更新索引文件 |
