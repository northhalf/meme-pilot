...
| **按文件名同步的增量刷新** | 启动和 `/refresh` 时使用的 v1.0 同步策略：对新增图片先按格式执行图片无损压缩，再追加索引记录；删除已不存在图片的索引记录；文件名仍存在的图片不重新 OCR，不检测同名覆盖；删除记录后保持其他已有 id 稳定，允许临时编号空洞；新增图片按文件名升序处理，并优先复用最小空洞 id |
| **关键词搜索** | 功能一：用户输入关键词，只对索引中的 OCR 文本做模糊匹配，返回对应表情包；不匹配文件名 |
