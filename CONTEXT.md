...
| **私聊** | v1.0 的基础会话形态：授权 QQ 用户与 Bot 一对一对话；支持所有命令（组 A：`/add`、`/addtag`、`/del`、`/ai`、`/edittext`、`/setspeaker`、`/refresh`；组 B：`/search`、`/help`、`/info`、普通文本） |
| **授权用户** | v1.0 中允许使用 Bot 的 QQ 用户；可以配置一个或多个，`/help`、`/search`、`/ai`、`/add`、`/addtag`、`/del`、`/edittext`、`/setspeaker`、`/refresh`、`/info` 都只对授权用户开放 |
...
| **群聊消息** | `/search`、`/help`、`/info`、普通文本（组 B）可通过群聊中 @bot 的方式触发；`/add`、`/addtag`、`/del`、`/ai`、`/edittext`、`/setspeaker`、`/refresh`（组 A）群聊中 @bot 调用时回复"此命令仅限私聊使用"。非授权用户在群聊中@bot 发送任何消息时静默忽略。 |
