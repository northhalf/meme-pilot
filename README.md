...
MemePilot 是一个部署在 Docker 中的 QQ 表情包机器人，帮你从本地表情包库中快速找到想要的表情包。`/search`、`/query`、`/rand`、`/sim`、`/help`、`/info` 和普通文本支持群聊 @bot 使用；`/add`、`/addtag`、`/del`、`/refresh`、`/ai`、`/edittext`、`/setspeaker` 仅限私聊。`/cancel` 私聊和群聊均可使用。
...
`/help`、`/search`、`/rand`、`/sim`、`/ai`、`/add`、`/addtag`、`/del`、`/edittext`、`/setspeaker`、`/refresh`、`/info`、`/cancel` 使用同一组授权用户白名单。非授权用户的私聊和群聊消息都会被静默忽略。

### 群聊支持
`/search`、`/query`、`/rand`、`/sim`、`/help`、`/info` 和普通文本支持在群聊中 @bot 触发。`/add`、`/addtag`、`/del`、`/ai`、`/refresh`、`/edittext`、`/setspeaker` 在群聊中 @bot 调用时会回复"此命令仅限私聊使用"。`/cancel` 私聊和群聊均可使用。
